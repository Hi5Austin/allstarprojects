...
var displayProjects = function(name,link,creator,type,des,img) {
    ...
    var html ="";
    html += "<a target='_blank' href='http://austincarvey.github.io/allstarprojects/project.html?projects_url=" + link + "'><div id='project' style='background-color:white;width:300px;height:600; padding:15px; border:1px solid #D3D3D3;border-radius:10px;margin:25px;'>";
...
allComs.on('child_added',function(snapshot) {
  ...
  if(coo.url === getURLParameter("projects_url")){
    $("#indie").append('<div style="text-align:left;" id="myComs"><a id="mname">' + coo.name + "</a><a id='name'> "+ ": " + coo.comments + '</a><br></div>');
  }
});
    ...
    projects.on('value',function(snapshot) {
      ...
      for(var key in stuff) {
        if(stuff[key].link === projects_url) {
          ...
          $("#dez").append('<a>by ' + stuff[key].creator + '</a><br>');
